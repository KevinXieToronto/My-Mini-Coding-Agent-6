...
import { defaultKeybindings, type KeybindingsManager } from "../keybindings.ts";
...
export class Editor implements EditorComponent {
	...
	insertTextAtCursor(text: string): void {
		...
		if (inserted.length === 1) {
			...
			this.lines.splice(this.cursorLine, 1, before + inserted[0], ...inserted.slice(1, -1), tail + after);
